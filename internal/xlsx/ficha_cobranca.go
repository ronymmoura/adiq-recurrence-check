...
func (wb *Workbook) AddFichaCobranca(ficha []sql.FichaCobranca) error {
	...
	headers := []ColHeader{
		...
		{Name: "Local Cobranca", Width: 10.0},
		{Name: "Data Competência", Width: 15.0},
		{Name: "Data Referencia", Width: 15.0},
		{Name: "Data Registro", Width: 15.0},
		{Name: "Valor", Width: 10.0},
		{Name: "Status Cobrança", Width: 10.0},
	}
	...
	for _, item := range ficha {
		...
		cell = row.AddCell()
		cell.SetStyle(style)
		cell.SetInt(item.SqLocalCobranca)
		...
		cell = row.AddCell()
		cell.SetStyle(style)
		cell.SetDateTime(item.DtRegistro)

		cell = row.AddCell()
		cell.SetStyle(style)
		cell.SetFloat(item.VlCobranca)

		cell = row.AddCell()
		cell.SetStyle(style)
		cell.SetString(item.IrStatusCobranca)
	}
